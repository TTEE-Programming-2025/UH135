...
/*心得
這次作業，我學會了使用 struct 來儲存每位學生的資料，這讓資料的管理變得更有條理。在
資料的輸入、排序及搜尋這部分，我更熟悉了如何操作陣列並利用for迴圈一一顯示內容。同時
在使用氣泡排序法排序學生總平均分時，我學會了比較與交換整筆結構資料。為了避免使用者輸
入錯誤，我這次在輸入分數和選項時加了不少 while 判斷，盡可能完善所有可能。
還有這次我把需要的不同功能全部都寫成函式，並特別宣告顯示在最前面，這樣除了讓主程式更
好撰寫、看起來更整齊外，我也能快速藉由vim指令跳到最前面檢查我寫的功能是否有缺漏，更簡
單快速的修改選單功能。*/
